...
import talex.zsw.baselibrary.view.TedPicker.adapter.SpacesItemDecoration;
...
public class ImagePickerActivity extends AppCompatActivity implements CameraHostProvider
{
	...
	private void initView()
	{
		...
		view_selected_photos_container.getViewTreeObserver()
			.addOnPreDrawListener( new ViewTreeObserver.OnPreDrawListener()
			{
				@Override public boolean onPreDraw()
				{
					...
					int selected_bottom_size =
						(int) getResources().getDimension( mConfig.getSelectedBottomHeight() );

					ViewGroup.LayoutParams params =
						view_selected_photos_container.getLayoutParams();
	...
	private void setSelectedPhotoRecyclerView()
	{
		...
		rc_selected_photos.addItemDecoration( new SpacesItemDecoration( Util.dpToPx( this, 5 ),
			SpacesItemDecoration.TYPE_VERTICAL ) );
	...
	public void addImage(final Uri uri)
	{
		...
		{
			String text = String.format( getResources().getString( R.string.max_count_msg ),
				mConfig.getSelectionLimit() );
