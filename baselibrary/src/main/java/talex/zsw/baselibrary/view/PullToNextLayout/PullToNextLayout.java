...
import talex.zsw.baselibrary.view.PullToNextLayout.anim.PullToNextAnimationI;
import talex.zsw.baselibrary.view.PullToNextLayout.anim.SimpleAnimation;
import talex.zsw.baselibrary.view.PullToNextLayout.base.SimpleAnimatorListener;
import talex.zsw.baselibrary.view.PullToNextLayout.observable.PullToNextDataObserver;
...
public class PullToNextLayout extends FrameLayout
{
	...
	private void init(AttributeSet attrs)
	{
		if (attrs != null)
		{
			TypedArray typedArray =
				getContext().obtainStyledAttributes(attrs, R.styleable.PullToNextLayout);
			bgColor = typedArray.getColor(R.styleable.PullToNextLayout_underLayoutColor,
				getResources().getColor(R.color.bg_pulltonext));
			topType = typedArray.getInt(R.styleable.PullToNextLayout_topType, 0);
			typedArray.recycle();
		}
		else
		{
			bgColor = getResources().getColor(R.color.bg_pulltonext);
		}
	...
	public void setStates(String[] states)
	{
		...
		{
			prompt =
				new PromptEntity(states[0], states[1], states[2], states[3], states[4], states[5]);
			if (currentPTE != null)
			{
				currentPTE.getPullToNextView().setPromptEntity(prompt);
			}
			if (previousPTE != null)
			{
				previousPTE.getPullToNextView().setPromptEntity(prompt);
			}
			if (nextPTE != null)
			{
				nextPTE.getPullToNextView().setPromptEntity(prompt);
			}
		}
	}
	...
	public PullToNextEntity newPullToNextView(int frameLayoutId)
	{
		...
		PullToNextView pullToNextView = new PullToNextView(getContext(), topType);
	...
	private void previous()
	{
		...
		Animator anim = simpleAnimation
			.getPullDownAnim(previousPTE.getPullToNextView(), currentPTE.getPullToNextView());
	...
	private void next()
	{
		...
		remove(previousPTE);
		Animator anim = simpleAnimation
			.getPullUpAnim(nextPTE.getPullToNextView(), currentPTE.getPullToNextView());
	...
	public void deleteCurrentItem()
	{
		...
		if (currentPTE.getPosition() == 0)
		{
			remove(previousPTE);
			Animator anim = simpleAnimation
				.getDeleteItemAnim(nextPTE.getPullToNextView(), currentPTE.getPullToNextView());
		...
		else
		{
			remove(nextPTE);
			Animator anim = simpleAnimation
				.getDeleteItemAnim(previousPTE.getPullToNextView(), currentPTE.getPullToNextView());
			anim.addListener(new SimpleAnimatorListener()
			{
