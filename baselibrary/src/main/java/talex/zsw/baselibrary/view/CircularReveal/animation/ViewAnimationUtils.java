package talex.zsw.baselibrary.view.CircularReveal.animation;
...
import static talex.zsw.baselibrary.view.CircularReveal.animation.RevealAnimator.CLIP_RADIUS;
...
public class ViewAnimationUtils
{
		...
		View view, int centerX, int centerY, float startRadius, float endRadius)
	{
		...
		ObjectAnimator reveal =
			ObjectAnimator.ofFloat( revealLayout, CLIP_RADIUS, startRadius, endRadius );
	...
	@Deprecated public static void liftingFromBottom(View view, float baseRotation, int duration)
	{
		...
		ViewPropertyAnimator.animate( view )
			.setInterpolator( new AccelerateDecelerateInterpolator() ).setDuration( duration )
			.rotationX( 0 ).translationY( 0 ).start();
	}
