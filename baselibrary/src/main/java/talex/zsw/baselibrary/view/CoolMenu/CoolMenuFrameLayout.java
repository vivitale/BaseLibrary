...
public final class CoolMenuFrameLayout extends FrameLayout
{
	...
	public CoolMenuFrameLayout(Context context, AttributeSet attrs, int defStyleAttr)
	{
		...
		num = array.getInteger( R.styleable.CoolMenuFrameLayout_CFL_num, 3 );
		mTitleColor = array.getColor( R.styleable.CoolMenuFrameLayout_CFL_titleColor,
			getResources().getColor( android.R.color.primary_text_light ) );
		mTitleSize = array.getDimension( R.styleable.CoolMenuFrameLayout_CFL_titleSize,
			getResources().getDimension( R.dimen.cl_title_size ) );
		mMenuIcon = array.getDrawable( R.styleable.CoolMenuFrameLayout_CFL_titleIcon );
		array.recycle();
		init();
	}
	...
	private void initAnim()
	{
		for(int i = 0; i < getChildCount(); i++)
		{
			...
			PropertyValuesHolder valuesHolderX =
				PropertyValuesHolder.ofFloat( "XFraction", 0, transX );
			PropertyValuesHolder valuesHolderY =
				PropertyValuesHolder.ofFloat( "YFraction", 0, transY );
			animator = ObjectAnimator.ofPropertyValuesHolder( child, valuesHolderX, valuesHolderY );
			animator.setInterpolator( mInterpolator );
			animator.setDuration( 300 );
			mOpenAnimators[i] = animator;

			PropertyValuesHolder valuesHolderXReverse =
				PropertyValuesHolder.ofFloat( "XFraction", transX, 1 );
			animator = ObjectAnimator.ofPropertyValuesHolder( child, valuesHolderXReverse );
			animator.setInterpolator( mInterpolator );
			animator.setDuration( 300 );
			mChosenAnimators[i] = animator;

			PropertyValuesHolder valuesHolderAlpha =
				PropertyValuesHolder.ofFloat( "menuAlpha", 1, 0 );
	...
	@UiThread public void setTitles(@NonNull List<String> titles)
	{
		for(int i = 0; i < num; i++)
		{
	...
	@UiThread public void setTitleByIndex(@NonNull String title, int index)
	{
		if(index > num - 1)
		{
	...
	@UiThread public void setTitleBackground(@NonNull List<Integer> resIds)
	{
		for(int i = 0; i < num; i++)
		{
			((TranslateLayout) getChildAt( i )).setBackgroundResource( resIds.get( i ) );
		}
	}

	@UiThread public void setTitleBackgroundByIndex(int resId, int index)
	{
		if(index > num - 1)
		{
			throw new IndexOutOfBoundsException();
		}
		((TranslateLayout) getChildAt( index )).setBackgroundResource( resId );
	}

	@UiThread public void setMenuIcon(int resId)
	{
		for(int i = 0; i < num; i++)
		{
	...
	@UiThread public void setMenuIconByIndex(int resId, int index)
	{
		if(index > num - 1)
		{
			throw new IndexOutOfBoundsException();
		}
		((TranslateLayout) getChildAt( index )).setMenuIcon( resId );
	}

	@UiThread public void setTitleHeight(int height)
	{
		for(int i = 0; i < num; i++)
		{
			((TranslateLayout) getChildAt( i )).setTitleHeight( height );
		}
	}

	@UiThread public LinearLayout getTitleLayoutByIndex(int index)
	{
		return ((TranslateLayout) getChildAt( index )).getTitleLayout();
	}
