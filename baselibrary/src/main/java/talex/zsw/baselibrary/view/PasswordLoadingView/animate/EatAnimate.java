...
public class EatAnimate extends BaseAnimate
{
	...
	@Override public void init(PsdLoadingView mPsdLoadingView)
	{
		...
		{
			@TargetApi(Build.VERSION_CODES.KITKAT) @Override public void handleMessage(Message msg)
			{
	...
	@TargetApi(Build.VERSION_CODES.HONEYCOMB) @Override public void startLoading()
	{
		isLeftTurn = true;
		super.startLoading();
		isBegining = true;
		...
		ValueAnimator eyeAnimator = ValueAnimator.ofFloat( 0, 1 ).setDuration( 100L );
	...
	@TargetApi(Build.VERSION_CODES.KITKAT) @Override public void onDraw(Canvas canvas)
	{
		super.onDraw( canvas );
		if(!isStop)
		{
			...
			if(isBegining)
			{
				for(int i = 0; i < textLength; i++)
				{
					...
					if(moveX < centerX)
					{
						canvas.drawText( String.valueOf( DOT ), 0, 1, moveX, startY, mPaint );
					}
				}
			}
			else
	...
	private void drawAmazed(Canvas canvas)
	{
		...
		canvas
			.drawCircle( amazedX + radius / 2.0f, centerY - radius / 2.0f, radius / 7.0f, mPaint );
		canvas
			.drawCircle( amazedX - radius / 2.0f, centerY - radius / 2.0f, radius / 7.0f, mPaint );
	}
