...
public class NavigationTabBar extends View implements ViewPager.OnPageChangeListener
{
	...
	private float ACTIVE_ICON_SCALE_BY = 0.35f;
	private final static float ICON_SIZE_FRACTION = 0.4f;

	private float TITLE_ACTIVE_ICON_SCALE_BY = 0.25f;
	private final static float TITLE_ICON_SIZE_FRACTION = 0.4f;
	private float TITLE_ACTIVE_SCALE_BY = 0.2f;
	...
	public NavigationTabBar(final Context context, final AttributeSet attrs, final int defStyleAttr)
	{
		...
		{
			setIsScaled(
				typedArray.getBoolean(R.styleable.NavigationTabBar_ntb_scaled, false)
			);
			setIsTitled(
	...
	{
		mIsScaled = isScaled;
		ACTIVE_ICON_SCALE_BY = 0.0f;
		TITLE_ACTIVE_ICON_SCALE_BY = 0.0f;
		TITLE_ACTIVE_SCALE_BY = 0.0f;
	}
		...
		if (mAnimatorListener == null)
		{
			mAnimatorListener = new Animator.AnimatorListener()
			{
				...
				public void onAnimationStart(final Animator animation)
				{
					if (mOnTabBarSelectedIndexListener != null && !mIsViewPagerMode)
					{
						mOnTabBarSelectedIndexListener
							.onStartTabSelected(mModels.get(mIndex), mIndex);
					}
				}
				...
				public void onAnimationEnd(final Animator animation)
				{
					...
					{
						mOnTabBarSelectedIndexListener
							.onEndTabSelected(mModels.get(mIndex), mIndex);
					}
				}
	...
	protected void onDraw(final Canvas canvas)
	{
		...
		for (int i = 0; i < mModels.size(); i++)
		{
			...
			final float titleTranslate =
				-model.mIcon.getHeight() + topTitleOffset - mTitleMargin * 0.5f;
		...
		{
			mIconsCanvas
				.drawRoundRect(mPointerBounds, mCornersRadius, mCornersRadius, mIconPointerPaint);
		}
