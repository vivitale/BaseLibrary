...
package talex.zsw.baselibrary.view.CommonAdapter.listviewanimations.view;
...
import talex.zsw.baselibrary.view.CommonAdapter.listviewanimations.itemmanipulation.SwipeOnTouchListener;
...
public class DynamicListView extends ListView
{
	...
	private Rect getChildViewRect(View parentView, View childView)
	{
		final Rect childRect =
			new Rect(childView.getLeft(), childView.getTop(), childView.getRight(),
				childView.getBottom());
		...
		switch (event.getAction() & MotionEvent.ACTION_MASK)
		{
			case MotionEvent.ACTION_DOWN:
				...
				if (mResIdOfDynamicTouchChild != 0)
				{
					...
					int position = pointToPosition(mDownX, mDownY);
					int childNum =
						(position != INVALID_POSITION) ? position - getFirstVisiblePosition() : -1;
					View itemView = (childNum >= 0) ? getChildAt(childNum) : null;
					View childView =
						(itemView != null) ? itemView.findViewById(mResIdOfDynamicTouchChild) :
							null;
					if (childView != null)
					{
	...
	{
		mIsParentHorizontalScrollContainer =
			(mResIdOfDynamicTouchChild == 0) ? isParentHorizontalScrollContainer : false;
		;
	}
	...
	private OnScrollListener mScrollListener = new OnScrollListener()
	{
							 ...
							 int totalItemCount)
		{
			...
			mPreviousFirstVisibleItem =
				(mPreviousFirstVisibleItem == -1) ? mCurrentFirstVisibleItem :
					mPreviousFirstVisibleItem;
			mPreviousVisibleItemCount =
				(mPreviousVisibleItemCount == -1) ? mCurrentVisibleItemCount :
					mPreviousVisibleItemCount;
