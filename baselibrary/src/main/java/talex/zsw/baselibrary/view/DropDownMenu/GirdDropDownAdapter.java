...
public class GirdDropDownAdapter extends BaseAdapter
{
	...
	public GirdDropDownAdapter(Context context, List<String> list, int checkedBG, int checkedTC,
	...
	@Override public View getView(int position, View convertView, ViewGroup parent)
	{
		...
		{
			convertView =
				LayoutInflater.from( context ).inflate( R.layout.item_list_drop_down, null );
	...
	private void fillValue(int position, ViewHolder viewHolder)
	{
		...
		if(checkItemPosition != -1)
		{
			...
			{
				viewHolder.mText
					.setTextColor( context.getResources().getColor( checkedTC ) );
				viewHolder.mText.setBackgroundResource( checkedBG );
				viewHolder.mText.setCompoundDrawablesWithIntrinsicBounds( null, null,
					context.getResources().getDrawable( R.mipmap.drop_down_checked ), null );
			}
			else
			{
