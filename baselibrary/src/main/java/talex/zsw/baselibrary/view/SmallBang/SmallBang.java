package talex.zsw.baselibrary.view.SmallBang;
...
public class SmallBang extends View
{
	...
	public void bang(final View view, float radius, SmallBangListener listener)
	{
		...
		{
			ViewAnimator.animate( view ).scale( 1f ).start();
		}
		ValueAnimator animator =
			ValueAnimator.ofFloat( 0f, 1f ).setDuration( (long) (ANIMATE_DURATION * 0.5f) );
	...
	private void bang()
	{
		ValueAnimator valueAnimator =
			ValueAnimator.ofFloat( 0, 1 ).setDuration( ANIMATE_DURATION );
		...
		else if(progress > P1)
		{
			...
			if(progress >= P2)
			{
				...
				for(int i = 0; i < dotList.size(); i += 2)
				{
					...
					circlePaint
						.setColor( evaluateColor( dot.startColor, dot.endColor, progress3 ) );
					...
					circlePaint
						.setColor( evaluateColor( dot2.startColor, dot2.endColor, progress3 ) );
					float x2 =
						(float) (r * Math.cos( i * 2 * Math.PI / DOT_NUMBER + 0.2 )) + centerX;
					float y2 =
						(float) (r * Math.sin( i * 2 * Math.PI / DOT_NUMBER + 0.2 )) + centerY;
