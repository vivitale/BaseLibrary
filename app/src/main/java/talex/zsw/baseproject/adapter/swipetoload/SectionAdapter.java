package talex.zsw.baseproject.adapter.swipetoload;
...
import talex.zsw.baseproject.test.Section;
import talex.zsw.baseproject.test.Character;
import talex.zsw.baseproject.util.CircleTransformation;
...
public class SectionAdapter extends BaseGroupAdapter<Section, Character>
{
	...
	@Override protected int getParentViewType(int groupPosition)
	{
		return -1;
	}
	...
	@Override protected Section getGroup(int groupPosition)
	{
	...
	@Override protected int getChildViewType(int groupPosition, int childPositionInGroup)
	{
		return 1;
	}
	...
	@Override protected Character getChild(int groupPosition, int childPosition)
	{
		return getGroup( groupPosition ).getCharacters().get( childPosition );
	}

	@Override
	protected View getChildView(final int groupPosition, final int childPosition, View convertView,
		ViewGroup parent)
	{
		...
		Picasso.with( parent.getContext() ).load( character.getAvatar() ).resize( size, size )
			.transform( new CircleTransformation( width ) ).into( holder.ivAvatar );
			...
			@Override public void onClick(View v)
			{
				if(mOnChildItemClickListener != null)
				{
					mOnChildItemClickListener
						.onChildItemClick( groupPosition, childPosition, character,
							finalConvertView );
				}
			}
		} );
			...
			@Override public boolean onLongClick(View v)
			{
				if(mOnChildItemLongClickListener != null)
				{
					return mOnChildItemLongClickListener
						.onClickItemLongClick( groupPosition, childPosition, character,
							finalConvertView );
				}
				return false;
			}
