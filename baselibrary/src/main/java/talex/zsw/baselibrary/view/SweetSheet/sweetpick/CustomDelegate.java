...
public class CustomDelegate extends Delegate
{
	...
	@Override
	public View createView(int SSCollor, int BGCollor)
	{

		View rootView = LayoutInflater.from(mParentVG.getContext())
			.inflate(R.layout.sweetsheet_layout_custom_sweet, null, false);

		mSweetView = (SweetView) rootView.findViewById(R.id.sv);
		mFreeGrowUpParentRelativeLayout =
			(FreeGrowUpParentRelativeLayout) rootView.findViewById(R.id.freeGrowUpParentF);
		mContentRL = (RelativeLayout) rootView.findViewById(R.id.rl);
		mContentRL.setBackgroundColor(BGCollor);

		sliderIm = (CRImageView) rootView.findViewById(R.id.sliderIM);
		mSweetView.reDraw(SSCollor);
		...
		if (mContentViewAnimationType == AnimationType.DuangLayoutAnimation)
		{
			...
			Animation animation = AnimationUtils.loadAnimation(mContentRL.getContext(),
				R.anim.sweetsheet_item_duang_show);
	...
	class AnimationImp implements SweetView.AnimationListener
	{
		...
		public void onEnd()
		{
			...
			if (mIsDragEnable)
			{
				...
				sliderIm.circularReveal(sliderIm.getWidth() / 2, sliderIm.getHeight() / 2, 0,
					sliderIm.getWidth());
			}
	...
	private void alphaAnimation()
	{
		...
		ObjectAnimator objectAnimator =
			ObjectAnimator.ofFloat(getContentRelativeLayout(), "alpha", 0, 1);
	...
	private void duangAnimation()
	{
		...
		Animation animation = AnimationUtils.loadAnimation(mContentRL.getContext(),
			R.anim.sweetsheet_item_duang_show2);
