...
import talex.zsw.baseproject.fragment.swipetoload.TwitterRecyclerFragment;
...
public class RecyclerCharactersAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>
{
	...
	@Override public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int i)
	{
		...
		switch(type)
		{
			...
			case TYPE_GROUP:
				...
				break;
			case TYPE_CHILD:
				onBindChildHolder( (ChildHolder) viewHolder, getGroupPosition( i ),
					getChildPosition( i ) );
				break;
		}
	}
	...
	private void onBindViewPagerHolder(ViewPagerHolder holder)
	{
		if(holder.viewPager.getAdapter() == null)
		{
			...
			holder.viewPager.setBackgroundDrawable(
				holder.itemView.getResources().getDrawable( R.mipmap.bg_viewpager ) );
	...
	private void onBindChildHolder(ChildHolder holder, int parentPosition, int childPosition)
	{
		...
		Picasso.with( holder.itemView.getContext() ).load( character.getAvatar() )
			.resize( size, size ).transform( new CircleTransformation( width ) )
