...
public class AccelerationProgress extends View implements LinearAnimation.LinearAnimationListener
{
	...
	private int mAccTextColor = Color.BLACK;
	...
	public AccelerationProgress(Context context, AttributeSet attrs, int defStyleAttr)
	{
		...
		TypedArray a = context.getTheme()
			.obtainStyledAttributes(attrs, R.styleable.AccelerationProgress, 0, 0);
		mAccBallRadius = a.getDimension(R.styleable.AccelerationProgress_AP_ball_radius, 14);
		mAccBallBackground =
			a.getColor(R.styleable.AccelerationProgress_AP_ball_background, Color.BLUE);
		mBigCircleStroke = a.getDimension(R.styleable.AccelerationProgress_AP_big_circle_stroke, 7);
		mBigCircleColor =
			a.getColor(R.styleable.AccelerationProgress_AP_big_circle_background, Color.RED);
		mDuration = a.getInt(R.styleable.AccelerationProgress_AP_duration, DEFAULT_DURATION);
		mTimeTextSize = a.getDimensionPixelSize(R.styleable.AccelerationProgress_AP_text_size, 20);
		mAccTextColor =
			a.getColor(R.styleable.AccelerationProgress_AP_text_color, Color.BLACK);
		a.recycle();
		init();
	}
	...
	private void init()
	{
		...
		mTextPaint.setColor(mAccTextColor);
	...
	private void drawAccBall(Canvas canvas)
	{
		...
		if (mNeedDrawTime)
		{
			RectF textRect = new RectF(x - mAccBallRadius, y - mAccBallRadius, x + mAccBallRadius,
				y + mAccBallRadius);
			Paint.FontMetrics fontMetrics = mTextPaint.getFontMetrics();
			float baseline = textRect.top +
				(textRect.bottom - textRect.top - fontMetrics.bottom + fontMetrics.top) / 2 -
				fontMetrics.top;
			String curTime = String
				.valueOf(animator.getDuration() / 1000 - animator.getCurrentPlayTime() / 1000);
	...
	private void drawHook(Canvas canvas)
	{
		...
		hookPath.addRoundRect(new RectF(i + i / 2, i + i / 2, i + i / 2 + 6, i + i + i / 2), 3f, 3f,
			Path.Direction.CCW);
		//        hookPath.addRect(new RectF(i, i + i, i + 15, i + i + 15f), Path.Direction.CCW);
		hookPath.addRoundRect(
			new RectF(i + i / 2, (i + i + i / 2) - 3f, i + i / 2 + i + i, i + i + i / 2 + 3f), 3f,
			3f, Path.Direction.CCW);
	...
	/**
	 * 获取大圆环的半径 = 整个View的宽的一半 - 小球半径 - 大圆环的厚度
	 */
